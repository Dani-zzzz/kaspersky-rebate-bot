...
async def main():
    ...
    secret_token = os.environ.get('WEBHOOK_SECRET')  # Необязательно, для безопасности
    ...
    app.router.add_post(f'/{token}', webhook)  # Путь вебхука с токеном для безопасности
    ...
    logger.info(f"Веб-сервер запущен на порту {os.environ.get('PORT', 8080)}")
    logger.info(f"Вебхук зарегистрирован: {webhook_url}/{token}")
