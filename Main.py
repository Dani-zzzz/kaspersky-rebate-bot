...
def load_products():
    ...
    Пробует UTF-8, при неудаче — CP1251 (Windows-1251)."""
    products = {}
    ...
    for encoding in ('utf-8', 'cp1251'):
        try:
            with open(filepath, 'r', encoding=encoding) as f:
                content = f.readlines()
            logger.info(f"Файл {filepath} прочитан в кодировке {encoding}")
            break
    ...
    if content is None:
        logger.error("Не удалось прочитать products.csv ни в UTF-8, ни в CP1251")
        return products
    ...
    if query.data == "new":
        ...
        await query.edit_message_text(
            text=f"✅ Статус: {context.user_data['status']}\n"
                 ...
                 "Шаг 4: Введите один или несколько 4-значных номеров продуктов через пробел:"
        )
        ...
        await query.edit_message_text(
            text=f"✅ Статус: {context.user_data['status']}\n"
                 ...
                 "Шаг 4: Продукт был поставлен вашей компанией в прошлый период?",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return SUPPLIED
...
async def supplied_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        text=f"✅ Статус: {context.user_data['status']}\n"
             ...
             "Шаг 5: Введите один или несколько 4-значных номеров продуктов через пробел:"
    )
    return PRODUCT_NUMBER
...
async def product_number_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Разбиваем ввод по пробелам, запятым, точкам с запятой
    numbers = re.split(r'[\s,;]+', user_input)
    numbers = [n.strip() for n in numbers if n.strip()]

    if not numbers:
        await update.message.reply_text("❌ Введите хотя бы один номер продукта.")
        return PRODUCT_NUMBER

    if len(numbers) > 30:
        await update.message.reply_text("❌ Слишком много номеров за раз. Максимум 30.")
        return PRODUCT_NUMBER
    ...
    results = []
    for num in numbers:
        if not num.isdigit() or len(num) < 3 or len(num) > 4:
            results.append(f"⚠️ `{num}` — некорректный номер")
            continue

        product = PRODUCTS_DB.get(num)
        ...
        if product:
            ...
            spec_match = False
            name = None  # названия нет — не выводим
        ...
        if name:
            results.append(f"`{num}` — {name} — **{total}%**")
        else:
            results.append(f"`{num}` — **{total}%**")

    # Формируем заголовок
    header = f"📋 Статус: **{status}** | Специализация: **{user_spec}**\n"
    if deal_type == "new":
        header += "Тип сделки: **новая продажа**\n"
    else:
        header += f"Тип сделки: **продление** (поставлен ранее: {'да' if supplied else 'нет'})\n"
